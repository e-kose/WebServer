...
/*   Updated: 2025/05/13 01:11:56 by menasy           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void  WebServer::sendResponse(pollfd& pollStruct, const std::string& status)
{
	...
	std::string	log = socketInfo(clientToAddrMap[fd], fd) + " " 
						...
						+ this->httpRequest->getRequestFile() 
						+ " " + status.substr(0,pos);
	...
	{
		std::string httpMethod = this->httpRequest->getMethod();
		if (httpMethod == "GET")
			response = HelperClass::createHttpResponse(status, "OK", "text/html", HelperClass::readHtmlFile(this->resultPath));
...
void WebServer::deleteMethod(pollfd& pollStruct)
{
	...
	for (size_t i = 0; i < locations.size(); ++i) {
		if (this->httpRequest->getPath() == locations[i].getPath())
		{
